...
from autoattack2 import checks
...
class AutoAttack():
    ...
    def run_standard_evaluation(self, x_orig, y_orig, bs=250, return_labels=False):
        ...
        n_cls = checks.check_range_output(self.get_logits, x_orig[:bs].to(self.device), y_orig[:bs].to(self.device),
            logger=self.logger)
        checks.check_dynamic(self.model, x_orig[:bs].to(self.device), y_orig[:bs].to(self.device), self.is_tf_model,
            logger=self.logger)
