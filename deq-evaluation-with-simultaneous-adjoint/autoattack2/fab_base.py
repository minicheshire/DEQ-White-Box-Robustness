...
from autoattack2.fab_projections import projection_linf, projection_l2,\
    projection_l1
...
class FABAttack():
    ...
    def get_diff_logits_grads_batch(self, imgs, yy, la):
        raise NotImplementedError("Virtual function.")

    def get_diff_logits_grads_batch_targeted(self, imgs, yy, la, la_target):
       raise NotImplementedError("Virtual function.")
    ...
    def attack_single_run(self, x, y=None, use_rand_start=False, is_targeted=False):
        ...
        while counter_restarts < 1:
            ...
            while counter_iter < self.n_iter:
                with torch.no_grad():
                    if is_targeted:
                        df, dg = self.get_diff_logits_grads_batch_targeted(x1, la2, la2, la_target2)
                    else:
                        df, dg = self.get_diff_logits_grads_batch(x1, la2, la2)
                    if self.norm == 'Linf':
                        dist1 = df.abs() / (1e-12 +
                                            dg.abs()
            ...
            if not self.targeted:
                for counter in range(self.n_restarts):
                    ...
                    if ind_to_fool.numel() != 0:
                        ...
                        acc_curr = self._predict_fn(adv_curr, y_to_fool).max(1)[1] == y_to_fool
                        if self.norm == 'Linf':
