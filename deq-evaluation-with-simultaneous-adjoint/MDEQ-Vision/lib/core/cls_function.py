...
#    LOWER = (0.0 - MEAN) / SIGMA
#    UPPER = (1.0 - MEAN) / SIGMA
#    STEP  = step_size / SIGMA
#    EPS   = epsilon / SIGMA
#    EPS   = EPS[None, :, None, None]
#    LOWER = LOWER[None, :, None, None]
#    UPPER = UPPER[None, :, None, None]
#    x_adv = data.detach() + (torch.from_numpy(np.random.uniform(-epsilon, epsilon, data.shape)).float().cuda().permute(0,2,3,1)/SIGMA).permute(0,3,1,2)
#    x_adv = torch.min(torch.max(x_adv, LOWER), UPPER)
#    all_data = data.detach()
    x_adv = data.detach() + torch.from_numpy(np.random.uniform(-epsilon, epsilon, data.shape)).float().cuda()
#    all_data = torch.cat([all_data, x_adv])
    with torch.enable_grad():
        for k in range(num_steps):
            x_adv.requires_grad_()
            if AttackGrad == "simul-adjoint-es-0":
                now_grad = None
                for kkk in range(8):
                    output = model(x_adv, target=target, train_step=-1, compute_jac_loss=False, spectral_radius_mode=False, writer=None, AttackGrad="simul-adjoint-{}".format(kkk))[0]
                    model.zero_grad()
                ...
                output = model(x_adv, target=target, train_step=-1, compute_jac_loss=False, spectral_radius_mode=False, writer=None, AttackGrad=AttackGrad)[0]
...
#            eta = (STEP * (x_adv.grad.sign()).permute(0,2,3,1)).permute(0,3,1,2)
            x_adv = x_adv.detach() + eta
            x_adv = torch.min(torch.max(x_adv, data - epsilon), data + epsilon)
            x_adv = torch.clamp(x_adv, 0, 1)
#            all_data = torch.cat([all_data, x_adv.detach()])
#            x_adv = torch.min(torch.max(x_adv, LOWER), UPPER)
    return x_adv
...
def validate_observe_train(config, val_loader, model, criterion, lr_scheduler, epoch, output_dir, tb_log_dir,
    ...
    from autoattack2 import AutoAttack
    forward_pass = lambda x,y: model(x, target=y, train_step=-1, compute_jac_loss=False, spectral_radius_mode=spectral_radius_mode, writer=None, AttackGrad=AttackGrad)[0]
...
#            gen_data_dir = os.path.join(output_dir, 'gen_data_real')
#            if not os.path.exists(gen_data_dir): os.mkdir(gen_data_dir)

            target = target.cuda(non_blocking=True)
            input = input.cuda()
            now_bsz = target.shape[0]
#            now_x_advs = torch.load(os.path.join(gen_data_dir, "gen_x_advs_{}".format(i)))[:now_bsz, :,:,:].cuda()
            ...
            aa_name_list = ["apgd-ce", "apgd-t", "fab-t", "square", "apgd-dlr", "fab", "apgd-ce-es-0", "apgd-t-es-0", "fab-t-es-0", "square-es-0", "apgd-dlr-es-0", "fab-es-0"]
...
#                print(x_advs.shape)
            ...
            for j in range(2):
                ...
                output, zz, yy_list_list = model(x=now_x_adv, train_step=-1, compute_jac_loss=False,
                                            spectral_radius_mode=spectral_radius_mode, writer=None, AttackGrad=TrainGrad) # attackgrad = traingrad !!

                _, preddd = output.topk(1, 1, True, True)
                preddd = preddd.t().squeeze()

                now_step = 0
                tot_step = len(yy_list_list)
#                print(tot_step)
                ...
                for yy_list in (yy_list_list if (not "es-0" in attack) else [now]):
                    ...
                    bad_loss = criterion(now_output, preddd)
                    ...
                    ret["bad_loss"].append(bad_loss.item())
                    ret["acc1"].append(now_prec1[0].item())
                    now_step += 1

#                prec1, prec5 = accuracy(output, target, topk=topk)
#                good_loss = criterion(output, target)
#                bad_loss = criterion(output, preddd)
                
#                lastt = vec2list(now_func(list2vec(yy_list_list[-1])), cutoffs)
#                ret["yy_list_list"].append([_.cpu() for _ in lastt])
#                ret["output_list"].append(output.cpu())
#                ret["good_loss"].append(good_loss.item())
#                ret["bad_loss"].append(bad_loss.item())
#                ret["acc1"].append(prec1[0].item())
                if j == 0:
                    rets["CLEAN"] = ret#.append(ret)
                else:
                    rets["PERTURBED"] = ret#.append(ret)

                msg = "Batch {}/{}, {}, OutputFinal: ".format(i, total_batch_num, "CLEAN" if j==0 else "PERTURBED") + "good loss {}\tbad loss {}\tAcc@1 {}".format(round(good_loss.item(), 4), round(bad_loss.item(), 4), round(now_prec1[0].item(), 4))
                logger.info(msg)
