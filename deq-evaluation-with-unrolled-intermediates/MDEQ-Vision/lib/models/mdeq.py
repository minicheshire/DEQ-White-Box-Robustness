...
class MDEQClsNet(MDEQNet):
    ...
    def forward(self, x, train_step=0, AttackGrad="unroll", **kwargs):
        y_list, jac_loss, y_list_list = self._forward(x=x, train_step=train_step, AttackGrad=AttackGrad, **kwargs)
...
#            del y_list
#            torch.cuda.empty_cache()
#            return None, None, [now2]#[list2vec(yy_list) for yy_list in y_list_list]
...
class MDEQSegNet(MDEQNet):
    ...
    def forward(self, x, train_step=0, isUnrollGrad=False, **kwargs):
        y, jac_loss, sradius = self._forward(x=x, train_step=train_step, isUnrollGrad=isUnrollGrad, **kwargs)
