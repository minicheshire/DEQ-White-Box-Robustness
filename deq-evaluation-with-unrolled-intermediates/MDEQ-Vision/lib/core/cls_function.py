    ...
    with torch.enable_grad():
        for k in range(num_steps):
            ...
            if AttackGrad == "Funroll-es-0":
                now_grad = None
                for kkk in range(8):
                    output = model(x_adv, train_step=-1, compute_jac_loss=False, spectral_radius_mode=False, writer=None, AttackGrad="Funroll-{}-1".format(kkk))[0]
                    model.zero_grad()
                ...
                output = model(x_adv, train_step=-1, compute_jac_loss=False, spectral_radius_mode=False, writer=None, AttackGrad=AttackGrad)[0]
                model.zero_grad()
                loss_adv = criterion(output, target)
                loss_adv.backward()
...
#                print(x_advs.shape)
            ...
            for j in range(2):
                ...
                output, zz, yy_list_list = model(x=now_x_adv, train_step=-1, compute_jac_loss=False,
                                            spectral_radius_mode=spectral_radius_mode, writer=None, AttackGrad=TrainGrad) # attackgrad = traingrad when testing !!
