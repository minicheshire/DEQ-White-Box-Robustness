...
#    log_file_setting = "train-{}-attack-{}".format("unroll" if args.isUnrollTrain else "real", "unroll" if args.isUnrollGrad else "real")
    ...
    if config.TEST.MODEL_FILE:
        ...
        """
        if not args.isUnrollTrain:
            model_state_file = os.path.join(final_output_dir, 'checkpoint_86.pth.tar')
        elif args.TrainGrad == "unroll":
            if "cls_mdeq_TINY_reg_anderson" in args.cfg:
                model_state_file = os.path.join(final_output_dir, 'unroll-checkpoint_50.pth.tar')
            elif "cls_mdeq_TINY_reg" in args.cfg:
                model_state_file = os.path.join(final_output_dir, 'unroll-checkpoint_52.pth.tar')
            elif "cls_mdeq_XL_reg2" in args.cfg:
                model_state_file = os.path.join(final_output_dir, 'unroll-checkpoint_106.pth.tar')
            elif "cls_mdeq_XL_reg" in args.cfg:
                model_state_file = os.path.join(final_output_dir, 'unroll-checkpoint_73.pth.tar')
            else:
                model_state_file = os.path.join(final_output_dir, 'unroll-checkpoint_68.pth.tar')
        else:
