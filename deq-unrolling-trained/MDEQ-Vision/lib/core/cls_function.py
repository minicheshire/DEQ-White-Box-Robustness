...
def pgd(model, data, target, criterion, epsilon, step_size, num_steps, isTrain=False):
    ...
    SIGMA = torch.tensor([0.2023, 0.1994, 0.2010]).cpu()
...
def train(config, train_loader, model, criterion, optimizer, lr_scheduler, epoch,
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if num_steps != 0 and global_steps > config.TRAIN.PRETRAIN_STEPS:
            ...
            if is_mixed:
                output, jac_loss0, _ = model(input, train_step=(lr_scheduler._step_count-1), 
                                            compute_jac_loss=compute_jac_loss,
                                            f_thres=f_thres, b_thres=b_thres, writer=writer)
                loss0 = criterion(output, target)
                jac_loss0 = jac_loss0.mean()

            x_adv = pgd(model, input, target, criterion, epsilon=epsilon, step_size=step_size, num_steps=num_steps, isTrain=True)
            model.train()
            output, jac_loss, _ = model(x_adv, train_step=(lr_scheduler._step_count-1), 
                                        compute_jac_loss=compute_jac_loss,
                                        f_thres=f_thres, b_thres=b_thres, writer=writer)

        else:
            if need_adv_pretrain:
